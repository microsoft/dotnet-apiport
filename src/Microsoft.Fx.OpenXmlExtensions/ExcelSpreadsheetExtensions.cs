...
namespace Microsoft.Fx.OpenXmlExtensions
{
	public static class OpenXmlExtensions
	{
		...
		internal static uint IncrementalUniqueId { get { return _GlobalId++; } }
		...
		public static Worksheet AddWorkheet(this SpreadsheetDocument spreadsheet, string name, CsvDocument sourceDocument)
		{
			var worksheet = spreadsheet.AddWorksheet(name);

			string[] headers = sourceDocument.Keys.ToArray();

			worksheet.AddRow(headers);
			foreach (var row in sourceDocument.Rows)
			{
				worksheet.AddRow(row.Values.ToArray());
			}
