...
namespace PortabilityService.WorkflowManagement
{
    ...
    public class WorkflowManager
    {
        ...
        {
            Debug.Assert(actions.Length - 1 > (int)currentMsg.Stage, "Workflow message Stage value must be within bounds of Actions array.");

            var action = actions[(int)currentMsg.Stage];

            Debug.Assert(action.CurrentStage == currentMsg.Stage, "Workflow message Stage must match Action's Stage.");
