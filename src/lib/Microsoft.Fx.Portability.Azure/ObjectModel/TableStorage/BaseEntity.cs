        ...
        private static readonly string[] HexStringTable = new string[]
{
    ...
    "F0", "F1", "F2", "F3", "F4", "F5", "F6", "F7", "F8", "F9", "FA", "FB", "FC", "FD", "FE", "FF"
        ...
        private static string ToHexString(byte[] arr)
        {
            ...
            {
                sb.Append(HexStringTable[arr[i]]);
            }

            return sb.ToString();
        }

        protected string ComputeHashCode(string data)
        {
            using (SHA256Managed hashGenerator = new SHA256Managed())
            {
                return ToHexString(hashGenerator.ComputeHash(UTF8Encoding.UTF8.GetBytes(data)));
            }
        }
    }
}
